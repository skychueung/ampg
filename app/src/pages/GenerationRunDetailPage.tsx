...
export default function GenerationRunDetailPage() {
  ...
  const duration = run?.created_at && run?.completed_at
    ...
    : undefined
  const completed = (field: keyof PeptideCandidate) =>
    peptides.filter((p) => p[field] !== null && p[field] !== undefined).length
  const nullCount = (field: keyof PeptideCandidate) =>
    peptides.filter((p) => p[field] === null || p[field] === undefined).length
  const outputDirectory = artifactDir || task?.artifact_dir || 'not available / 当前接口未返回'
  const logPath = artifactDir ? `${artifactDir}/stdout.log, ${artifactDir}/stderr.log` : 'not available / 当前接口未返回'
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-[12px] text-[#991B1B]">
          ...
          {run.backend === 'SERVER_PRODUCTION' &&
            ' Server Production output remains computational ranking; toxicity and hemolysis remain null unless real scorers are connected.'}
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-y-4 gap-x-6 text-[13px]">
          ...
          <div>
            <span className="text-[#6B7280] block text-[12px] mb-0.5">Updated</span>
            <span className="text-[#111827]">{formatDateTime(task?.updated_at || run.completed_at || run.created_at)}</span>
          </div>
          <div>
          ...
          {run.backend === 'SERVER_PRODUCTION' && (
            <>
              ...
              <div>
                ...
                <span className="text-[#111827]">真实接口字段 / 当前接口未返回则不伪造</span>
              </div>
            </>
          )}
          <div className="col-span-3 grid grid-cols-1 gap-3 md:grid-cols-3">
            <div className="rounded-[6px] border border-[#E5E7EB] bg-[#F9FAFB] p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">Output Directory</span>
              <span className="break-all font-mono text-[12px] text-[#111827]">{outputDirectory}</span>
            </div>
            <div className="rounded-[6px] border border-[#E5E7EB] bg-[#F9FAFB] p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">Log Path</span>
              <span className="break-all font-mono text-[12px] text-[#111827]">{logPath}</span>
            </div>
            <div className="rounded-[6px] border border-[#E5E7EB] bg-[#F9FAFB] p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">Peptide Count</span>
              <span className="text-[#111827]">{peptides.length} / {run.count}</span>
            </div>
          </div>
          <div className="col-span-3 grid grid-cols-1 gap-3 md:grid-cols-5">
            <div className="rounded-[6px] border border-[#E5E7EB] bg-white p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">amp_score</span>
              <span className="text-[#111827]">{completed('amp_score')} / {peptides.length}</span>
            </div>
            <div className="rounded-[6px] border border-[#E5E7EB] bg-white p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">mic_saureus</span>
              <span className="text-[#111827]">{completed('mic_saureus')} / {peptides.length}</span>
            </div>
            <div className="rounded-[6px] border border-[#E5E7EB] bg-white p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">mic_ecoli</span>
              <span className="text-[#111827]">{completed('mic_ecoli')} / {peptides.length}</span>
            </div>
            <div className="rounded-[6px] border border-[#E5E7EB] bg-white p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">toxicity_risk</span>
              <span className="text-[#111827]">{nullCount('toxicity_risk')} null / {peptides.length}</span>
            </div>
            <div className="rounded-[6px] border border-[#E5E7EB] bg-white p-3">
              <span className="text-[#6B7280] block text-[12px] mb-0.5">hemolysis_risk</span>
              <span className="text-[#111827]">{nullCount('hemolysis_risk')} null / {peptides.length}</span>
            </div>
          </div>
          {task?.message && (
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-y-3 gap-x-6 text-[13px]">
            ...
            {!task.error_message && (
              <div className="col-span-3">
                <span className="text-[#6B7280] block text-[12px] mb-0.5">Error</span>
                <span className="text-[#9CA3AF]">not available / 当前接口未返回</span>
              </div>
            )}
            {task.artifact_dir && (
              <div className="col-span-3">
              ...
              <tbody>
                {peptides.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-center text-[12px] text-[#111827]">{p.amp_score ?? 'null'}</td>
                    <td className="px-3 py-2.5 text-center text-[12px] text-[#111827]">{p.mic_ecoli ?? 'null'}</td>
                    <td className="px-3 py-2.5 text-center text-[12px] text-[#111827]">{p.mic_saureus ?? 'null'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
