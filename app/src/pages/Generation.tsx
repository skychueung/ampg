...
export default function Generation() {
  ...
  const [modelBackend, setModelBackend] = useState<ModelBackend>('LOCAL_DEMO')
  ...
  const handleGenerate = useCallback(async () => {
    ...
    /* BLOCKED logic per backend tier */
    if (modelBackend === 'LOCAL_REAL_SMOKE' && count > 2) {
    ...
    if (modelBackend === 'LOCAL_DEMO' && count > 5) {
      setGenStatus('BLOCKED')
      setTaskId(`task-${Date.now()}`)
      setGenerated([])
      setRunResult(null)
  ...
  return (
    <Layout title={t('generation.title') as string} subtitle={t('generation.subtitle') as string}>
      <div className="space-y-6">
        ...
        >
          <h2 className="text-[20px] font-semibold text-[#111827]">{t('generation.title') as string}</h2>
          <p className="text-[14px] text-[#6B7280] mt-1">
            {t('generation.subtitle') as string}
          </p>
        </motion.div>
        ...
        <AnimatePresence>
          {genStatus !== 'IDLE' && (
            ...
            >
              ...
              {/* LOCAL_REAL_SMOKE notice */}
              {modelBackend === 'LOCAL_REAL_SMOKE' && (genStatus === 'PENDING' || genStatus === 'RUNNING') && (
                <div className="mb-4 p-3 rounded-[6px] bg-[#FEF3C7] border border-[#FDE68A] flex items-start gap-2.5">
